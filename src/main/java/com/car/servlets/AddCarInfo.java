...
/**
 * Servlet implementation class AddCarInfo
 */
...
public class AddCarInfo extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		System.out.println("触发添加车辆信息servlet");
		   request.setCharacterEncoding("UTF-8");
			...
			boolean flag=cBiz.AddCarInfo(tcar);
			if(flag){
				System.out.println("添加车辆信息成功");
				ajax.setStatusCode(200);
				ajax.setMessage("添加车辆信息成功");
				...
				System.out.println("添加失败,已存在数据无法插入");
				ajax.setStatusCode(300);
				ajax.setMessage("添加失败,已存在数据无法插入");	
				ajax.setCallbackType("closeCurrent");
				JSONObject json= new JSONObject(ajax);
				response.setContentType("text/html; charset=utf-8");
				System.out.println(json);
